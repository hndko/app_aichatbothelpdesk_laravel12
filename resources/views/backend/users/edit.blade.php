...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">Perbarui profil, kontak, serta kewenangan akses untuk pengguna ini</p>
            </div>
        </div>
    ...
    <div class="p-4 rounded-2xl bg-indigo-50/80 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800/60 flex items-start gap-3 text-indigo-900 dark:text-indigo-200">
        ...
        <div class="text-xs leading-relaxed">
            <strong class="font-bold block text-sm mb-0.5">Catatan Perubahan Password:</strong>
            Kosongkan kolom password di bawah ini jika Anda tidak ingin mengubah sandi pengguna. Jika ingin langsung mengembalikan ke default (<code class="font-mono font-bold">password</code>), gunakan tombol cepat <strong>🔑 Reset Pass</strong> di halaman daftar pengguna.
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="role" id="role" class="bg-gray-50 border @error('role') border-red-500 @else border-gray-300 @enderror text-gray-900 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-all font-bold">
                        ...
                        <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>🟣 Administrator IT</option>
                    </select>
                    @error('role')
                        <p class="text-xs font-bold text-rose-600 dark:text-rose-400 mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                    @foreach($categories as $cat)
                        <label class="flex items-center p-3.5 rounded-xl border border-gray-200 dark:border-gray-700 hover:bg-indigo-50/50 dark:hover:bg-gray-700/50 cursor-pointer transition-all gap-3 bg-gray-50/50 dark:bg-gray-800">
                            <input type="checkbox" name="specializations[]" value="{{ $cat->id }}" class="w-4 h-4 text-indigo-600 bg-white border-gray-300 rounded focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" {{ in_array($cat->id, old('specializations', $userSpecs)) ? 'checked' : '' }}>
